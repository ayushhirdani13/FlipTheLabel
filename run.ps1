...
$test_batch_size = if ($args[5]) { $args[5] } else { '2048' }

$drop_rate = if ($mode -eq 'normal') {0} else {$drop_rate}
$num_gradual = if ($mode -eq 'normal') {0} else {$num_gradual}

# Run python script
python -u main.py --dataset=$dataset --model=$model --mode=$mode --drop_rate=$drop_rate --num_gradual=$num_gradual --test_batch_size=$test_batch_size > "log/$dataset/$model-$mode`_$drop_rate-$num_gradual.log" 2>&1