...
class NCFData(data.Dataset):
    def __init__(
        ...
        self.flips = [1 for _ in range(len(features))]
    ...
    def ng_sample(self):
        ...
        flips_ps = self.flips
        flips_ng = [0 for _ in range(len(self.features_ng))]

        self.noisy_or_not_fill = self.noisy_or_not + [
            1 for _ in range(len(self.features_ng))
        ]
    ...
    def get_advantage_ratio(self):
        try:
            flips = np.array(self.flips)
            noisy_or_not = np.array(self.noisy_or_not)
            true_mask = (flips == 0) & (noisy_or_not == 0)
    ...
    def __getitem__(self, idx):
        ...
        noisy_or_not = (
            self.noisy_or_not_fill if self.is_training != 2 else self.noisy_or_not
        )
        flips = self.flips_fill if self.is_training != 2 else self.flips
        ...
        return user, item, label, noisy_label, flip, idx
